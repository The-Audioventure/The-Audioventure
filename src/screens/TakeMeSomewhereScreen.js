...
const TakeMeSomewhere = ({ navigation }) => {
  ...
  const labelVisibleInit = Object.keys(AppConstants.placeThemes).reduce(
    (acc, currKey) => {
      acc[currKey] = false;
      return acc;
    },
  ...
  const [labelVisible, labelVisibleDispatch] = useReducer(
    labelVisibleReducer,
    {}
  );
    ...
    if (elem === "Back") {
      ...
      return (
        <IconButton
          ...
          onMouseLeave={() =>
            labelVisibleDispatch({ type: elem, payload: false })
          }
        />
      );
    }
